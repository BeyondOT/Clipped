...
const avatarFilefilter = (req, file, cb) => {
  if (
    ...
    req.ValidationError = "File type Error. Please upload a file with the right format (png, jpg, jpeg)."  
    return cb(null, false, req.ValidationError);
  }

  if (file.size > avatarMaxSize) {
    req.ValidationError = "File too large. Please upload a file that is less than 1Mb." 
    return cb(null, false, req.ValidationError);
  }
  return cb(null, true);
};
const avatarUpload = multer({ storage: avatarStorage, fileFilter: avatarFilefilter });
...
const postImageUpload = multer({ storage: postImageStorage, fileFilter: postImageFilefilter });
